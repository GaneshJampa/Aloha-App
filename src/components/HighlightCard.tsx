        ...
        <div className={`${index == '0' ? "ml-4" : ""}${index == '1' ? "mx-4" : ""}${index == '2' ? "mr-4" : ""} lg:w-full w-[24rem] overflow-hidden bg-white rounded-lg shadow-[0_0_16px_rgba(0,128,128,0.16)]`}>
            <Link href={`/activities/${highlight.title}`} title={highlight.title}>
                ...
                <div className="lg:px-4 py-5 px-6">
                    <p className="mb-2 text-xl font-bold text-[#008080]">
                        {highlight.title}</p>
                    <p className="mb-2 text-[#001A1A] leading-5">
                        {highlight.description}
                    </p>
                    <div className="flex justify-end">
                        <button title={highlight.title}>
                            <svg className={`${index == '1' ? "-mt-5" : ""} lg:mt-0 w-10 p-3 bg-[#E6F2F2] rounded-full text-[#008080] hover:bg-[#008080] hover:text-white transition ease-in-out duration-200`} viewBox="0 0 16 16" fill="none">
